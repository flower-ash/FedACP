...
class MemReporter():
    ...
    """
    # Optional[torch.nn.Module]表示参数model可以是torch.nn.Module类型的值，也可以是None
    def __init__(self, model: Optional[torch.nn.Module] = None):
        self.tensor_name = {}
        # self.tensor_name是字典，用于存储张量的名称。它的键是张量的唯一标识符（通过id(param)获取），值是张量的名称字符串。
        self.device_mapping = defaultdict(list)
        # defaultdict(list)创建了一个默认值为列表的字典。也就是说，当你访问self.device_mapping中不存在的键时，它会自动创建一个空列表作为默认值。
        # self.device_mapping是一个默认字典（defaultdict(list)），用于存储设备和张量之间的映射关系。每个设备对应一个列表，列表中包含与该设备相关的张量。
        self.device_tensor_stat = {}# self.device_tensor_stat是一个字典，用于存储每个设备的张量统计信息
        self.name_idx = 0# self.name_idx是一个计数器，用于为未知张量进行编号。

        # 通过遍历model的命名参数（model.named_parameters()），将参数与名称关联起来，并存储在tensor_names字典中。
        ...
        # 使用tensor_names字典中的参数和名称，将每个参数的唯一标识符（id(param)）作为键，
        # 将名称列表用"+"连接为字符串后作为值，存储在self.tensor_name字典中。这样就建立了参数和名称之间的映射关系。
        for param, name in tensor_names.items(): # tensor_names.items()是一个方法调用，它将返回tensor_names中的所有键值对作为元组的列表。
